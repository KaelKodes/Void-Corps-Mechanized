...
public sealed class ClaimArenaLayout
{
	...
	[
		// ===== Gen 3.0 / MEDIUM floor (96×96) =====

		// Open disputed rock — sparse procedural cover across the Medium pad.
		new ClaimArenaLayout
		{
			...
			Covers =
			[
				// Mid stays relatively open — one light hardpoint only
				new(CoverKind.ConcreteBarrier, new Vector3(4f, 0f, -6f), 40f),
				new(CoverKind.ShippingContainer, new Vector3(-8f, 0f, 6f), 65f, 0.95f),
				// SE approach (player)
				...
				// South edge pocket
				new(CoverKind.BarrierRow, new Vector3(8f, 0f, -34f), 5f),
				new(CoverKind.OilTank, new Vector3(-12f, 0f, -36f), 10f, 0.8f)
			],
			CratePositions =
			[
		...
		// Abandoned relay pad — tunnels stretch to edges; junction stays mid.
		new ClaimArenaLayout
		{
			...
			[
				...
				// Outer ramps onto far tunnel roofs
		...
		// Cold dock shelf — mid overpass kept; freight + ledge pushed to the rim.
		new ClaimArenaLayout
		{
			...
			Covers =
			[
				// Mid cut — overpass + ramps (only dense mid structure)
				...
				// Soft cover on approach lanes (spread N/S, not hugging bridge)
				...
				new(CoverKind.BarrierRow, new Vector3(8f, 0f, -36f), 8f),
				new(CoverKind.ConcreteBarrier, new Vector3(-6f, 0f, 36f), -8f),

				// Corner / edge freight pockets
				...
				// Skyline rim
				new(CoverKind.Skyscraper, new Vector3(-43f, 0f, -8f), 0f, 1.0f),
				new(CoverKind.Skyscraper, new Vector3(-43f, 0f, 16f), 12f, 0.85f),
				new(CoverKind.Skyscraper, new Vector3(43f, 0f, -14f), -8f, 1.1f),
				new(CoverKind.Skyscraper, new Vector3(43f, 0f, 18f), 5f, 0.75f),
				new(CoverKind.Skyscraper, new Vector3(-12f, 0f, -43f), 90f, 0.9f),
				new(CoverKind.Skyscraper, new Vector3(14f, 0f, 43f), -90f, 0.8f)
			],
			CratePositions =
			[
